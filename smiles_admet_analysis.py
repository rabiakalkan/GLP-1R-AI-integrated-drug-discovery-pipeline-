...
def save_results_to_excel(df, output_dir):
    ...
    # All results (sorted by ADMET score)
    ...
    #Molecules with high QED scores (QED > 0.7)
    high_qed = df[df['QED'] > 0.7].sort_values('ADMET_Score', ascending=False)
    high_qed.to_excel(writer, sheet_name='High QED (>0.7)', index=False)
    
    # Molecules with high BBB scores (BBB > 0.6)
    high_bbb = df[df['BBB_Score'] > 0.6].sort_values('BBB_Score', ascending=False)
    high_bbb.to_excel(writer, sheet_name='High BBB (>0.6)', index=False)
    
    # Molecules with high absorption scores (Absorption > 0.7)
    high_abs = df[df['Absorption_Score'] > 0.7].sort_values('Absorption_Score', ascending=False)
    high_abs.to_excel(writer, sheet_name='High Absorbtion (>0.7)', index=False)
    
    # Molecules with low toxicity scores (Toxicity < 0.3)
    low_tox = df[df['Toxicity_Score'] < 0.3].sort_values('ADMET_Score', ascending=False)
    low_tox.to_excel(writer, sheet_name='Low Toxicity (<0.3)', index=False)
    
    # Molecules with good metabolic profiles (Metabolic > 0.6)
    good_met = df[df['Metabolic_Score'] > 0.6].sort_values('Metabolic_Score', ascending=False)
    good_met.to_excel(writer, sheet_name='Good Metabolism (>0.6)', index=False)
    
    # Drug-like molecules (ADMET > 0.6 ve QED > 0.6)
    drug_like = df[(df['ADMET_Score'] > 0.6) & (df['QED'] > 0.6)].sort_values('ADMET_Score', ascending=False)
    drug_like.to_excel(writer, sheet_name='Drug-like', index=False)
    
    # Adjust column widths on each page
